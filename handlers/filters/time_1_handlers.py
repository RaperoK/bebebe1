...
async def time_1_from(message: types.Message):
    value = message.text
    if try_parse_int(value) and int(value) >= const.TIME1['min'] and int(value) < filters.time_1['to']:
...
async def time_1_to(message: types.Message):
    value = message.text
    if try_parse_int(value) and int(value) > filters.time_1['from'] and int(value) <= const.TIME1['max']:
